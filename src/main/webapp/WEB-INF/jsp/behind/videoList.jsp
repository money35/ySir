    ...
    <div class="row">
        ...
        <div class="col-md-6">
            ...
            <form class="navbar-form navbar-right" action="${pageContext.request.contextPath}/video/list" method="post">
                <input type="text" name="title" class="form-control" placeholder="标题" value="${condition.title}">
                <div class="btn-group">
                    <button type="button" id="speakerName"
                            class="btn btn-primary dropdown-toggle" data-toggle="dropdown"
                            aria-haspopup="true" aria-expanded="false">
                        <c:forEach items="${speakerList}" var="speaker">
                            <c:if test="${speaker.id == condition.speakerId}">
                                ${speaker.speakerName}
                            </c:if>
                        </c:forEach>
                        <c:if test="${empty condition.speakerId || condition.speakerId == 0}">
                            --请选择老师--
                        </c:if>
                    ...
                    <input type="hidden" name="speakerId" id="speakerId" value="${condition.speakerId}"/>
                </div>
                <div class="btn-group">
                    <button type="button" id="courseName"
                        ...
                        <c:forEach items="${courseList}" var="course">
                            <c:if test="${course.id == condition.courseId}">
                                ${course.courseTitle}
                            </c:if>
                        </c:forEach>
                        <c:if test="${empty condition.courseId || condition.courseId == 0}">
                            --请选择课程--
                        </c:if>
                        <span class="caret"></span>
                    </button>
                    ...
                    <input type="hidden" name="courseId" id="courseId" value="${condition.courseId}"/>
                </div>
...
<%--            <li><a href="${pageContext.request.contextPath}/video/list">1</a></li>--%>
<%--            <li><a href="${pageContext.request.contextPath}/video/list">2</a></li>--%>
<%--            <li><a href="${pageContext.request.contextPath}/video/list">3</a></li>--%>
<%--            <li><a href="${pageContext.request.contextPath}/video/list">4</a></li>--%>
<%--            <li><a href="${pageContext.request.contextPath}/video/list">5</a></li>--%>
<%--            <li><a href="${pageContext.request.contextPath}/video/list">&raquo;</a></li>--%>
<%--        </ul>--%>
    ...
    <nav aria-label="..." class="navbar-right" style="margin-right: 15px">
        <ul class="pagination">
            ...
            <li <c:if test="${pageInfo.pageNum==1}">class="disabled"</c:if>>
                <a href="${pageContext.request.contextPath}/video/list?pageNum=${pageInfo.pageNum-1}&title=${condition.title}&speakerId=${condition.speakerId}&courseId=${condition.courseId}" aria-label="Previous">
                    <span aria-hidden="true">«</span></a></li>
            <c:forEach items="${pageInfo.navigatepageNums}" var="num">
                <c:if test="${pageInfo.pageNum==num}"><li class="active"><a href="#">${num} <span class="sr-only">(current)</span></a></li></c:if>
                <c:if test="${pageInfo.pageNum!=num}"><li><a href="${pageContext.request.contextPath}/video/list?pageNum=${num}&title=${condition.title}&speakerId=${condition.speakerId}&courseId=${condition.courseId}">
                        ${num}</a></li></c:if>

            </c:forEach>
            <li <c:if test="${pageInfo.pageNum==pageInfo.pages}" >class="disabled"</c:if>
            ><a href="${pageContext.request.contextPath}/video/list?pageNum=${pageInfo.pageNum+1}&title=${condition.title}&speakerId=${condition.speakerId}&courseId=${condition.courseId}" aria-label="Next">
                <span aria-hidden="true">»</span></a></li>
        </ul>
    </nav>
